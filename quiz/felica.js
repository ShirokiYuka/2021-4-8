...
const felica1Event = new CustomEvent('felica1', { detail: '1' });
const felica2Event = new CustomEvent('felica2', { detail: '2' });
const felica3Event = new CustomEvent('felica3', { detail: '3' });
const felicaOffEvent = new CustomEvent('felicaOff', { detail: 'felicaOff' });
...
// Felicaカード
var cards = {};
cards["012e4cd44ac12d"] = "1";  // FeliCa Card 1
cards["012e4ce3500d42"] = "2";  // FeliCa Card 2
cards["012e4ce3500d32"] = "3";  // FeliCa Card 3
...
async function session(device) {
  ...
  if( currentIDm != idmStr ) {
      ...
      if(action == "1") {
        // 「1」のFeliCaカードをかざした
        document.dispatchEvent(felica1Event);
      }
      else if(action == "2") {
        // 「2」のFeliCaカードをかざした
        document.dispatchEvent(felica2Event);
      }
      else if(action == "3") {
        // 「3」のFeliCaカードをかざした
        document.dispatchEvent(felica3Event);
      }
      else {
        // なにもしない
      }
    ...
    if( currentIDm != "" ) {
        ...
        document.dispatchEvent(felicaOffEvent);
    }
  }
}

document.addEventListener('webUSBclick', async () => {
  let device;
  try {
