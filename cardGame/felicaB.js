...
const felicaB1Event = new CustomEvent('felicaB1', { detail: 'B1' });
const felicaB2Event = new CustomEvent('felicaB2', { detail: 'B2' });
const felicaB3Event = new CustomEvent('felicaB3', { detail: 'B3' });
const felicaBOffEvent = new CustomEvent('felicaBOff', { detail: 'felicaBOff' });
...
// Felicaカード
var cards = {};
cards["012e4cd44ac12d"] = "B1";  // FeliCa Card 1
cards["012e4ce3500d42"] = "B2";  // FeliCa Card 2
cards["012e4ce3500d32"] = "B3";  // FeliCa Card 3
...
async function session(device) {
  ...
  if( currentIDm != idmStr ) {
      ...
      if(action == "B1") {
        // 「1」のFeliCaカードをかざした
        document.dispatchEvent(felicaB1Event);
      }
      else if(action == "B2") {
        // 「2」のFeliCaカードをかざした
        document.dispatchEvent(felicaB2Event);
      }
      else if(action == "B3") {
        // 「3」のFeliCaカードをかざした
        document.dispatchEvent(felicaB3Event);
      }
      else {
        // なにもしない
      }
    ...
    if( currentIDm != "" ) {
        ...
        document.dispatchEvent(felicaBOffEvent);
    }
  }
}

document.addEventListener('webUSBBclick', async () => {
  let device;
  try {
